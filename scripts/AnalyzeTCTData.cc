...
Float_t AnalyzeTCTData::CalcSignalNoise(TH1F* his)
{
    ...
    right = his->GetXaxis()->FindBin(_tA);
...
//Float_t AnalyzeTCTData::CalcMaxSlope(TH1F* his) //Calculate the Max Slope in leading edge of signal
//{
//    Float_t threshold = 10;
//    Int_t n = his->GetNbinsX();
//    Float_t sigMax = his->GetMaximum(); //Maximum Signal
//    Float_t sigLow  = sigMax * (threshold/100); //10% of signal
//    Float_t sigHigh  = sigMax * (100-threshold/100); //90% of signal
//    Float_t maxSlope = 0;
//    Float_t signal=0, signalAbove=0, timeAbove=0, time=0, slope=0;
//
//    for(Int_t i=0;i<n-1;++i)
//    {
//        signal = his->GetBinContent(i);
//        if(signal<=sigLow)
//            continue;
//        if(signal>=sigHigh)
//            break;
//        signalAbove = his->GetBinContent(i+1);
//
//        if(signal == signalAbove)
//        {
//            timeAbove += _tct->t0 + (i+1) * _tct->dt;
//            continue;
//        }
//        time = _tct->t0 + i * _tct->dt;
//
//        //timeAbove = his->GetXaxis()->GetBinCenter(i+1);
//        //cout<<timeAbove-time<<endl;
//        //cout<<signalAbove-signal<<endl;
//
//        slope = (signalAbove-signal)/(timeAbove-time);
//        if(slope>maxSlope)
//            maxSlope = slope;
//    }
//    return maxSlope;
...
Float_t AnalyzeTCTData::CalcSlope(TH1F* his, Float_t threshold) //Calculate slope of the leading edge
{
    ...
    Float_t signal, slope, s1, s2, t1, t2;
...
Float_t AnalyzeTCTData::CalcSlope(TH1F* his) //Calculate slope of the leading edge
{
    ...
    Int_t n = his->GetNbinsX();
    Float_t sigMax = his->GetMaximum(); //Maximum Signal
    Float_t thres1 = sigMax * (threshold-10)/100;
    Float_t thres2 = sigMax * (threshold+10)/100;
...
//
//    Bool_t foundLow = kFALSE, foundHigh = kFALSE;
//
...
//            continue;
//        }
//        if(signal > thres1 && foundLow == kFALSE)
//        {
//            s1 = signal;
//            //time1 = _tct->t0 + i * _tct->dt;
//            //time1 = his->GetXaxis()->GetBinCenter(i);
//            //cout<<time1<<endl;
//            foundLow = kTRUE;
//            continue;
//        }
...
//            continue;
//        }
//        if(signal > thres2 && foundHigh == kFALSE)
//        {
//            s2 = signal;
//            //time2 = _tct->t0 + i * _tct->dt;
//            foundHigh = kTRUE;
//        }
//        if(foundLow && foundHigh)
//        {
//            cout<<Form("DeltaSig: %lf \tDeltaTime:%lf", s2-s1, t2-t1)<<endl;
//            slope = (s2-s1)/(t2-t1);
//            return slope;
//        }
//    }
