...
AnalyzeTCTData::AnalyzeTCTData(const char* inFile)
{
  ...
  if(FindWFShift())   //Find the shift in the waveforms and Correct the Baseline using tA
    ...
    {
      cout<<"\033[1;31m[  ERROR]\033[0m Can not determine the start of the Signal!! \n";
      cout<<"\033[1;35m[WARNING]\033[0m start of the signal set to default Value (0.)!! \n";
    }
...
AnalyzeTCTData::AnalyzeTCTData(const char* inFile, Float_t threshold)
{
  ...
  _fileName = ((TObjString *)(_filePath->At(_filePath->GetEntries()-1)))->String();
  _fileName = _fileName.ReplaceAll(".tct", "");
  _outFile = "/home/ashish/workPhd/tctAnalysisMaster/figures/"+_fileName;
...
AnalyzeTCTData::~AnalyzeTCTData()
{
  ...
  delete[] _sigRiseTime;
  delete[] _sigRiseTimeError;
  delete[] _sigMaxSlope;
  delete[] _sigMaxSlopeError;
  delete[] _sigSlewRate;
  delete[] _sigSlewRateError;
  delete[] _sigSlope;
  delete[] _sigSlopeError;
  delete[] _sigJitter;
  delete[] _sigJitterError;
  delete[] _sigTOA;
  delete[] _sigTOAError;
  delete[] _sigTOT;
  delete[] _sigTOTError;
...
void AnalyzeTCTData::GetActiveChannels()
{
  ...
  if(_nAC == 0)
    {
      cout<<"\033[1;31m[  ERROR]\033[0m No Active Channels Found!! \n";
      cout<<"\033[1;32m[ STATUS]\033[0m Exiting Program... \n";
      exit(0);
    }
  else
    printf("\033[1;34m[MESSAGE]\033[0m %d Active Channel Found \n", _nAC);
  ...
  printf("\033[1;34m[MESSAGE]\033[0m Active Channels: ");
  for(Int_t iCh = 0; iCh < _nAC; ++iCh)
    printf("CH%d \n", _aCH[iCh]);
}
...
Bool_t AnalyzeTCTData::FindWFShift()
{
  ...
  for(Int_t i=0;i<_events; ++i)
    {
      if(i==0)
	printf("\033[1;32m[ STATUS]\033[0m Estimating signal start Time (%d Waveforms).... \n",_events);
  ...
  if(maxAmp < _threshold)
    {
      printf("\033[1;33m[   INFO]\033[0m No Signal Found!!\n");
  ...
  for(Int_t i=0; i< _tct->nPoints; ++i)
    {
      ...
      if(amplitude > maxAmp*0.5)
        {
	  _tA = (time/1e-9)-1.5; //1ns before the 50% of Max Signal
	  printf("\033[1;33m[   INFO]\033[0m Start of the Signal ==> %0.2f ns\n", _tA);
	  return true;
        }
    }
  _tA = 0.;
...
void AnalyzeTCTData::CorrectBaseline()
{
  ...
  Float_t tA=0.;  //time denoting the start of the pulse
  
  printf("\033[1;32m[ STATUS]\033[0m Initiating Baseline correction (%d Waveforms).... \n",_events);
  ...
  printf("\033[1;34m[MESSAGE]\033[0m Baseline Correction Finished!!! \n");
}

void AnalyzeTCTData::CalcNoise()
{
  // Function calculates the noise in the baseline of all wafeforms
  printf("\033[1;32m[ STATUS]\033[0m Calculating the noise in the Baseline.... \n");
  ...
  if(_noise < 1)
    {
      _isAveraged = true;
      printf("\033[1;34m[MESSAGE]\033[0m Waveforms were averaged in the OSCILLOSCOPE \n");
    }
  else
    {
      _isAveraged = false;
      printf("\033[1;34m[MESSAGE]\033[0m Waveforms were not averaged in the OSCILLOSCOPE \n");
    }
    
  if(_isAveraged)
    _noise *= sqrt(_nAvOsc);

  printf("\033[1;34m[MESSAGE]\033[0m Noise Estimation Finished ==> %f mV!!! \n", _noise);
  return;
}
...
void AnalyzeTCTData::CalculateWaveformProperties()
{
  ...
  printf("\033[1;32m[ STATUS]\033[0m Calculating Waveform Properties...\n");
  TH1F *signal;
  ...
  for(Int_t i=0;i<_nAC; ++i)
	...
	_sigRiseTime[i][j] = CalcRiseTime(signal, &_sigRiseTimeError[i][j]);
	_sigSlewRate[i][j] = CalcSlewRate(signal, &_sigSlewRateError[i][j]);
            
	if(_sigSlewRate[i][j] != 0)
	  {
	    _sigJitter[i][j] = (_sigNoise[i][j]/_sigSlewRate[i][j])*1000; //ps
	    _sigJitterError[i][j]= (_sigJitter[i][j]/_sigSlewRate[i][j]) * 1000 * _sigSlewRateError[i][j];
	  }
	else
	  {
	    _sigJitter[i][j] = 0; //ps
	    _sigJitterError[i][j] = 0; //ps
	  }
...
{
  // Function calculates the signal properties of all events
  printf("\033[1;32m[ STATUS]\033[0m Calculating Signal Properties...\n");
  TH1F *signal;
  ...
  for(Int_t i=0;i<_nAC; ++i)
	...
	_sigRiseTime[i][j] = CalcRiseTime(signal, &_sigRiseTimeError[i][j]);
	_sigSlope[i][j] = CalcSlope(signal, &_sigSlopeError[i][j]);
	_sigMaxSlope[i][j] = CalcMaxSlope(signal, &_sigMaxSlopeError[i][j]);
...
{  
  ...
  */
  ...
  switch(operation)
    ...
    case 11:
      printf("\033[1;31m[404 ERROR]\033[0m Working on it!!\n");
      break;
    case 16:     PlotAxisSweep(operation);      break;
    default:
      printf("\033[1;31m[  ERROR]\033[0m Could not Resolve an Operation to perform...\n");
      printf("\033[1;31m[WARNING]\033[0m Check the File. Exiting Program!!\n");
      exit(0);
    }
  return;
}
...
void AnalyzeTCTData::SaveSignalShape()
{
  ...
  cout<<"\033[1;32m[ STATUS]\033[0m Saving Signal...\n";
...
Float_t AnalyzeTCTData::CalcRiseTime(TH1F* his, Float_t* error)
{
  ...
  for(Int_t i=0; i<_tct->nPoints; ++i)
    {
      ...
      if(his->GetXaxis()->GetBinCenter(i+1) < 0. || his->GetXaxis()->GetBinCenter(i+1) > 30.)
	continue;
  ...
  xData.clear();
  yData.clear();
  *error = TMath::Sqrt(TMath::Power(_deltaT/t1, 2) + TMath::Power(_deltaT/t2, 2));
  return t2-t1;
}
...
Float_t AnalyzeTCTData::CalcCFD(TH1F *his, Float_t thr)
{
  ...
  for(Int_t i=0; i<_tct->nPoints; ++i)
    {
      // Push the signal part only into the vector
      // 30ns of the signal will be used
      if(his->GetXaxis()->GetBinCenter(i+1) < 0. || his->GetXaxis()->GetBinCenter(i+1) > 30.)
	continue;
      xData.push_back(his->GetXaxis()->GetBinCenter(i+1));
      yData.push_back(his->GetBinContent(i+1));
...
Float_t AnalyzeTCTData::CalcSlope(TH1F* his, Float_t* error) //Calculate slope of the leading edge
{
  ...
  Float_t thres1 = sigMax * (threshold-5)/100;
  Float_t thres2 = sigMax * (threshold+5)/100;  
  Float_t t1 = LinearInterpolation(xData, yData, thres1);
  Float_t t2 = LinearInterpolation(xData, yData, thres2);
  Float_t dV = thres2 - thres1;
  Float_t dT = t2-t1;
  Float_t slope = dV/dT;
  Float_t noise = CalcSignalNoise(his);
  Float_t errdT = TMath::Sqrt(TMath::Power(_deltaT, 2) + TMath::Power(_deltaT, 2));
  Float_t errdV = TMath::Sqrt(TMath::Power(noise, 2) + TMath::Power(noise, 2));
  *error = slope * TMath::Sqrt(TMath::Power(errdV/dV, 2) +TMath::Power(errdT/dT, 2));
  return slope;
}

Float_t AnalyzeTCTData::CalcSlope(TH1F* his, Float_t threshold, Float_t* error) //Calculate slope of the leading edge
{
  Int_t n = his->GetNbinsX();
  Float_t sigMax = his->GetMaximum(); //Maximum Signal
...
Float_t AnalyzeTCTData::CalcMaxSlope(TH1F* his, Float_t* error) //Calculate the Max Slope in leading edge of signal
{
  Float_t maxSlope = 0, slope = 0;
  Float_t thrLow = 10, thrHigh = 90;
  Float_t errSlope = 0;
  for(Int_t i=thrLow; i<=thrHigh;++i)
    {
      slope = CalcSlope(his, i, &errSlope);
      if(slope>maxSlope)
	{
	  maxSlope = slope;
	  *error = errSlope;
...
Float_t AnalyzeTCTData::CalcSlewRate(TH1F* his, Float_t* error) //Calculate the Max Slope in leading edge of signal
{
...
{
  cout<<"\033[1;32m[ STATUS]\033[0m Plotting Axis(X, Y, X, V1, V2) Sweep...\n";
  ...
  cout<<"\033[1;34m[MESSAGE]\033[0m Finished Plotting Axis(X, Y, X, V1, V2) Sweep!!!\n";
  return;
}

void AnalyzeTCTData::PlotMaps(Int_t mapType)
{
  cout<<"\033[1;32m[ STATUS]\033[0m Plotting Position Sensitive Maps...\n";
  ...
  for(Int_t iCh = 0; iCh<_nAC; ++iCh)
    {
      cout<<"\033[1;32m[ STATUS]\033[0m Processing CH "<<_aCH[iCh]<<std::flush;
      for(j = 0; j<pY; ++j)
	for(i = 0; i<pX; ++i)
	  {
	    _index = _tct->GetIndex(*s[0], *s[1], *s[2], *s[3], *s[4]);
	  ...
	  cout<<"\r\033[1;32m[ STATUS]\033[0m Finished: "<<msg[mode]<<" Map!!   "<< std::flush;
        }
    }
  cout<<"\n\033[1;34m[MESSAGE]\033[0m Finished Plotting Position Sensitive Maps!!!\n";
  return;
}

void AnalyzeTCTData::GetFocus()
{
  cout<<"\033[1;32m[ STATUS]\033[0m Finding Focus of the LASER...\n";
  ...
  delete _canvas;
  //PlotMaps(operation);
  cout<<"\033[1;34m[MESSAGE]\033[0m Finished finding focus of the LASER!!!\n";
  return;
}
...
/*
  Bool_t AnalyzeTCTData::FindWFShift()
  {
  printf("\033[1;32m[ STATUS]\033[0m Finding signal start Time (%d Waveforms).... \n",_tct->_totEvents);
  ...
  printf("\033[1;33m[   INFO]\033[0m : Start of the Signal at %f ns\n", _tA);
  return true;
  }
  }
  _tA = 0.;
...
/*
  ...
  // if(i%100==0)
  // 	printf("\033[1;32m[ STATUS]\033[0m Total Events Processed: %d\n", i);
